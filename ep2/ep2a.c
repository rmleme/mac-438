...
//            norte, de maneira concorrente, dando prioridade aos carros que
//            vao do sul para o norte.
...
void espera_passagem_aberta_para_norte(int i)
{
  count(1,i);
  sem_wait(&e);
  if (ns > 0)          // Tem carro na contra-mao
  {
    dn++;
    sem_post(&e);
...
void espera_passagem_aberta_para_sul(int i)
{
  count(1,i);
  sem_wait(&e);
  if (nn > 0)          // Tem carro na contra-mao
  {
    ds++;
    sem_post(&e);
...
int main(int argc, char *argv[])
{
  ...
  media_esp_sul = (double) tempos_sul / (2 * qtos_carros_inic *
                  qtas_travessias);
  printf("\nTempo medio de espera para atravessar do sul para o norte: %f\n",
         media_esp_norte);
  printf("Tempo medio de espera para atravessar do norte para o sul: %f\n",
         media_esp_sul);
